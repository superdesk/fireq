...
(echo name=$lxc mount_src=$(pwd); curl -s https://raw.githubusercontent.com/superdesk/fireq/master/files/superdesk-dev/lxc-init) | sudo bash
(echo host=$lxc; curl -s https://raw.githubusercontent.com/superdesk/fireq/master/files/superdesk-dev/install) | ssh root@$lxc
...
```sh
lxc=sdtest path=~/superdesk
(echo name=$lxc mount_src=$path; curl -s https://raw.githubusercontent.com/superdesk/fireq/master/files/superdesk-dev/lxc-init) | sudo bash
(echo host=$lxc testing=1; curl -s https://raw.githubusercontent.com/superdesk/fireq/master/files/superdesk-dev/install) | ssh root@$lxc
